...
try {
 await page.goto(process.env.TOWN_URL||'http://localhost:4173/?test',{waitUntil:'domcontentloaded'});await wait(()=>window.__town);await page.screenshot({timeout:120000,path:'tests/willowbrook-day.png'});checks.push('High quality daytime rendering');
 ...
 await page.locator('#settings').click();await page.selectOption('#daytime','580');await page.selectOption('#quality','low');await page.locator('#back').click();await page.setViewportSize({width:900,height:680});
 ...
 await tp(-13,10.5);await page.keyboard.press('KeyE');await tp(-13,7.2);await wait(()=>!window.__town.state().roofs[2].visible);await page.screenshot({timeout:120000,path:'tests/willowbrook-interior.png'});checks.push('Furnished interior rendering');
 ...
 assert.deepEqual(errors,[]);await writeFile('tests/visual-results.json',JSON.stringify({date:new Date().toISOString(),checks,errors},null,2));console.log(`${checks.length} additional checks passed.`);
