...
const scene=new THREE.Scene();scene.background=new THREE.Color(0xbcd8d6);scene.fog=new THREE.Fog(0xbcd8d6,45,110);
...
renderer.setPixelRatio(Math.min(devicePixelRatio,1.75));renderer.shadowMap.enabled=true;renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.outputColorSpace=THREE.SRGBColorSpace;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.12;
...
const sun=new THREE.DirectionalLight(0xffe1b5,3.1);sun.position.set(-22,35,20);sun.castShadow=true;sun.shadow.mapSize.set(2048,2048);Object.assign(sun.shadow.camera,{left:-42,right:42,top:42,bottom:-42,near:1,far:100});sun.shadow.normalBias=.055;sun.shadow.bias=-.0001;scene.add(sun);scene.add(sun.target);
...
const shadow=new THREE.Mesh(new THREE.CircleGeometry(.45,24),new THREE.MeshBasicMaterial({color:0x41543e,transparent:true,opacity:.17,depthWrite:false}));shadow.rotation.x=-Math.PI/2;scene.add(shadow);
...
const petalGeo=new THREE.BufferGeometry();const petalPositions=new Float32Array(120*3);for(let i=0;i<120;i++){petalPositions[i*3]=(Math.random()-.5)*66;petalPositions[i*3+1]=Math.random()*9+1;petalPositions[i*3+2]=(Math.random()-.5)*65;}petalGeo.setAttribute('position',new THREE.BufferAttribute(petalPositions,3));const petals=new THREE.Points(petalGeo,new THREE.PointsMaterial({color:0xfff0c4,size:.08,transparent:true,opacity:.7}));scene.add(petals);
...
const keys=new Set<string>();let yaw=.17,pitch=.45,distance=16.5,vertical=0,grounded=true,walkTime=0,elapsed=0,daylight=1,paused=false,dialogue=false,hasMoved=false,nearest: {kind:'npc';npc:Resident}|{kind:'door';door:Door}|{kind:'seed';index:number}|null=null;
...
function updateCamera(dt:number,instant=false){
 ...
 const rayDirection=desired.clone().sub(focus);const rayLength=rayDirection.length();cameraRay.set(focus,rayDirection.normalize());let safe=rayLength;
 for(const b of world.buildings){if(Math.abs(player.position.x-b.x)<b.w/2&&Math.abs(player.position.z-b.z)<b.d/2){b.roof.visible=false;const room=new THREE.Box3(new THREE.Vector3(b.x-b.w/2+.25,-100,b.z-b.d/2+.25),new THREE.Vector3(b.x+b.w/2-.25,100,b.z+b.d/2-.25));const exit=cameraRay.ray.intersectBox(room,new THREE.Vector3());if(exit)safe=Math.min(safe,Math.max(.9,exit.distanceTo(focus)-.25));continue;}b.roof.visible=true;
 ...
 desired.copy(focus).addScaledVector(rayDirection,safe);desired.y=Math.max(.5,desired.y);camera.position.lerp(desired,instant?1:1-Math.exp(-dt*9));camera.lookAt(focus);
}
...
function updateLight(){const hour=minutes/60;daylight=THREE.MathUtils.smoothstep(Math.sin((hour-6)/24*Math.PI*2),-.15,.5);const sky=new THREE.Color(0x172d48).lerp(new THREE.Color(0xbcd8d6),daylight);const golden=Math.max(0,1-Math.abs(hour-17.7)/2.1)*daylight;sky.lerp(new THREE.Color(0xe6baa0),golden*.45);(scene.background as THREE.Color).copy(sky);(scene.fog as THREE.Fog).color.copy(sky);hemi.intensity=.5+daylight*1.65;sun.intensity=.12+daylight*2.7;sun.color.set(0xffe4bd).lerp(new THREE.Color(0xffb075),golden*.65);sun.position.set(-25*Math.cos(hour/24*Math.PI*2),Math.max(6,12+Math.sin((hour-6)/24*Math.PI*2)*25),20);world.windowMat.emissiveIntensity=.08+(1-daylight)*1.2;world.lamps.forEach(l=>{l.intensity=(1-daylight)*6;l.visible=renderer.shadowMap.enabled&&daylight<.5&&l.position.distanceTo(player.position)<20;});$('#sun').textContent=daylight>.4?'☀':'☾';const h=Math.floor(minutes/60);$('#time').textContent=`${h%12||12}:${String(Math.floor(minutes%60)).padStart(2,'0')} ${h<12?'AM':'PM'}`;$('#day').textContent=`Day ${String(days).padStart(2,'0')}`;}
